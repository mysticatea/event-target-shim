...
class EventTarget {
    ...
    // Implementation
    dispatchEvent(event0) {
        const list = $$2(this)[String(event0.type)];
        if (list == null) {
            return true;
        }
        const event = event0 instanceof Event ? event0 : EventWrapper.wrap(event0);
...
function upsertEventAttributeListener(target, type, callback) {
    const list = ensureListenerList($$2(target), String(type));
...
function removeEventAttributeListener(target, type) {
    const list = $$2(target)[String(type)];
